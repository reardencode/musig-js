...
exports.signAgg =
  exports.partialVerifySync =
  exports.partialVerify =
  exports.partialSignSync =
  exports.partialSign =
  exports.nonceGenSync =
  exports.nonceGen =
  exports.addTweaks =
  exports.keyAggSync =
  exports.keyAgg =
...
function mod(a, b = secp256k1_1.CURVE.P) {
  const result = a % b;
  return result >= _0n ? result : b + result;
}
function isWithinCurveOrder(num) {
  return _0n < num && num < secp256k1_1.CURVE.n;
}
function isValidFieldElement(num) {
  return _0n < num && num < secp256k1_1.CURVE.P;
}
...
function normalizePublicKey(publicKey) {
  if (publicKey instanceof secp256k1_1.Point) {
    publicKey.assertValidity();
    return publicKey;
  } else {
    return secp256k1_1.Point.fromHex(publicKey);
  }
}
...
function normalize33b(p) {
  if (p instanceof secp256k1_1.Point) return p.toRawBytes(true);
...
class MusigKeyAggCache {
  constructor(
    publicKeyHash,
    secondPublicKeyX,
    publicKey = secp256k1_1.Point.ZERO,
    parity = false,
    tweak = _0n,
    _coefCache = new Map()
  ) {
  ...
  static *fromPublicKeys(pubKeys, sort = true) {
    ...
    const secondPublicKey =
      secondPublicKeyIndex >= 0 ? publicKeys[secondPublicKeyIndex] : secp256k1_1.Point.ZERO;
  ...
  *coefficient(publicKey) {
  ...
  addTweaks(tweaks, tweaksXOnly) {
    ...
    for (let i = 0; i < tweaks.length; i++) {
      if (!hasEvenY(publicKey) && tweaksXOnly[i]) {
        parity = !parity;
        tweak = secp256k1_1.CURVE.n - tweak;
        publicKey = publicKey.negate();
      }
      publicKey = secp256k1_1.Point.BASE.multiplyAndAddUnsafe(publicKey, tweaks[i], _1n);
      if (!publicKey) throw new Error('Tweak failed');
      tweak = mod(tweak + tweaks[i], secp256k1_1.CURVE.n);
    }
  ...
  static fromHex(hex) {
    ...
    const cache = new MusigKeyAggCache(
      ...
      secp256k1_1.Point.fromHex(bytes.subarray(0, 33)),
      bytes[97] === 0x01,
...
function normalizeNonceArg(p) {
  ...
  if (p instanceof secp256k1_1.Point) p = p.x;
  if (typeof p === 'number') {
...
function* musigNonceGen(
  sessionId = secp256k1_1.utils.randomBytes(),
  privateKey,
  message,
  aggregatePublicKey,
  extraInput
  ...
  for (let i = 0; i < 2; i++) {
    ...
    publicNonce.set(secp256k1_1.Point.fromPrivateKey(k).toRawBytes(true), i * 33);
  }
...
function* musigNonceProcess(aggNonce, message, cache) {
  ...
  const challenge = mod(bytesToNumber(challengeHash), secp256k1_1.CURVE.n);
  let sPart = _0n;
  if (cache.tweak) {
    sPart = mod(challenge * cache.tweak, secp256k1_1.CURVE.n);
    if (!hasEvenY(cache.publicKey)) {
      sPart = secp256k1_1.CURVE.n - sPart;
    }
  }
  return new MusigProcessedNonce(
    !hasEvenY(finalNonce),
...
function* musigPartialVerifyInner(sig, publicKey, publicNonce, cache, processedNonce) {
  ...
  const ver = publicKey.multiplyAndAddUnsafe(
    rj,
    mod(processedNonce.challenge * a, secp256k1_1.CURVE.n),
    _1n
  );
  if (!ver) return false;
  const sG = secp256k1_1.Point.BASE.multiply(sig);
...
function* musigPartialSign(message, privKey, nonce, aggNonce, keyAggCache) {
  ...
  const publicNonce = nonce.publicNonce || [
    secp256k1_1.Point.fromPrivateKey(privateNonces[0]),
    secp256k1_1.Point.fromPrivateKey(privateNonces[1]),
  ];
  if (processedNonce.finalNonceHasOddY) {
    for (let i = 0; i < privateNonces.length; i++) {
      privateNonces[i] = secp256k1_1.CURVE.n - privateNonces[i];
    }
  }
  const publicKey = secp256k1_1.Point.fromPrivateKey(privateKey);
  const a = yield* cache.coefficient(publicKey);
  if ((hasEvenY(publicKey) !== cache.parity) !== hasEvenY(cache.publicKey)) {
    privateKey = secp256k1_1.CURVE.n - privateKey;
  }
  const ad = mod(privateKey * a, secp256k1_1.CURVE.n);
  const ead = mod(processedNonce.challenge * ad, secp256k1_1.CURVE.n);
  const bk2 = mod(privateNonces[1] * processedNonce.coefficient, secp256k1_1.CURVE.n);
  const sig = mod(ead + privateNonces[0] + bk2, secp256k1_1.CURVE.n);
...
async function keyAgg(publicKeys, opts = {}) {
...
function keyAggSync(publicKeys, opts = {}) {
...
function addTweaks(keyAggCache, tweaks, tweaksXOnly) {
...
function partialVerify(sig, message, publicKey, publicNonce, aggNonce, keyAggCache, session) {
  return callAsync(
    musigPartialVerify(sig, message, publicKey, publicNonce, aggNonce, keyAggCache, session)
  );
}
exports.partialVerify = partialVerify;
function partialVerifySync(sig, message, publicKey, publicNonce, aggNonce, keyAggCache, session) {
  return callSync(
    musigPartialVerify(sig, message, publicKey, publicNonce, aggNonce, keyAggCache, session)
  );
}
...
function signAgg(sigs, session) {
  ...
  return concatBytes(
    ...
    numTo32b(
      normalizedSigs.reduce(
        (prev, cur) => mod(prev + cur, secp256k1_1.CURVE.n),
        processedNonce.sPart
      )
    )
  );
}
...
function callSync(gen) {
  let result = gen.next();
  while (!result.done) {
    if (typeof secp256k1_1.utils.sha256Sync !== 'function')
      throw new Error('utils.sha256Sync is undefined, you need to set it');
    if (result.value.type === 'tagged') {
      result = gen.next(
        secp256k1_1.utils.taggedHashSync(result.value.tag, ...result.value.messages)
      );
    } else if (result.value.type === 'sha256') {
      result = gen.next(secp256k1_1.utils.sha256Sync(...result.value.messages));
    }
  }
  return result.value;
}
async function callAsync(gen) {
  ...
  while (!result.done) {
    if (result.value.type === 'tagged') {
      result = gen.next(
        await secp256k1_1.utils.taggedHash(result.value.tag, ...result.value.messages)
      );
    } else if (result.value.type === 'sha256') {
      result = gen.next(await secp256k1_1.utils.sha256(...result.value.messages));
    }
  }
  return result.value;
}
